...
RSpec.describe TwitterTimelineHub do
  ...
  describe "#call" do
    it "collaborates with the twitter_client to get a timeline and delivers results" do
      ...
      expect(result).to have_attributes(tweets: [], status: :ok)
  ...
  context "when the timeline is NOT found" do
    it "returns a result with an empty array and a not_found status" do
      twitter_client = instance_double(Twitter::REST::Client)
  ...
  context "when the timeline is unauthorized" do
    context "when the timeline is forbidden" do
      it "returns a result with an empty array and a forbidden status" do
        twitter_client = instance_double(Twitter::REST::Client)
        allow(twitter_client).to receive(:user_timeline)
          .with("screen_name", count: 5)
    ...
    context "when the unauthorized error refers to an invalid token" do
      it "throws an exception" do
        ...
        allow(twitter_client).to receive(:user_timeline)
          .with("screen_name", count: 5)
          .once { fail Twitter::Error::Unauthorized, "Invalid or expired token" }

        twitter_timeline_hub = TwitterTimelineHub.new(twitter_client)

        expect { twitter_timeline_hub.call("screen_name", count: 5) }
  ...
  def build_tweet_double(user_name:, mention_name:, text:, created_at:)
    ...
    mention = instance_double(Twitter::Entity::UserMention, screen_name: mention_name)
    instance_double(
      Twitter::Tweet,
      user: user,
      text: text,
