...
describe('renderOneTweet', () => {
  ...
  it.only('transforms a tweet object into an HTML string', () => {
    const tweet = { created_at: '2016-01-04', text: '<p>Hi!</p>'};

    fixture.innerHTML = renderOneTweet(tweet);

    expect(fixture.querySelectorAll('.tweet > header').length).to.equal(1);
    expect(fixture.querySelectorAll('.tweet > header')[0].innerText).to.equal('2016-01-04');
